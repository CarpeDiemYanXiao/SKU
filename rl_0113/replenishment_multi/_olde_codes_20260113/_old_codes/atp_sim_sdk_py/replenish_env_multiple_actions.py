...
class ReplenishEnv(gym.Env):
    def __init__(
        ...
        ###day,rts_days,leadtime,multiplier,真实值,预测值
        self.rolling_input = [self.current_day, self.rts_days, 2, 5, 1, [0, 0, 0, 0, 0, 0]]
        ##state列表:idx,day,current_stock,rts_qty_agg,bind_qty_agg
        self.state = []
        self.skus = self.load_data()
    ...
    def step(self, actions):
        ...
        for i, sku in enumerate(self.skus):
            ...
            sku_stat = self.gen_rolling_input(sku.model_id, actions[i])
