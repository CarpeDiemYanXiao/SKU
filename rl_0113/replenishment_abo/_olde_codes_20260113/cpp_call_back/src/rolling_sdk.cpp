...
ErrorCode step_one_sku(SkuInputInfo *input, bool evaluate, ReplenishmentCallback callback, int overnight_key) {
    ...
    // 调用callback获取multiplier
    float multiplier;
    try {
        multiplier = callback ? callback(input->day_index) : 1.0f;
    } catch (...) {
        debug_print("Exception in callback, using default multiplier");
        multiplier = 1.0f;
    }
    ...
    try {
        ...
        for (size_t i = 1; i < total_days; i++) {
            ...
            // 获取新的multiplier
            try {
                multiplier = callback ? callback(temp.day_index + 1) : 1.0f;
            } catch (...) {
                multiplier = 1.0f;
            }
