...
class ReplenishEnv(gym.Env):
    ...
    def cal_reward_safety_stock(self, sku,sku_id, overnight_qty,leadtime_sale_qty):
        bind_reward = self.coverage_weight * sku.lead_time_bind
        # overnight_penalty = self.overnight_weight[1] * sum(overnight_qty)
        res_penalty = self.rts_weight * sku.estimate_rts_qty  # 到货后未来14天仍未消耗的
        ...
        # 计算本次补货，过夜天数对应的补货量
        overnight_per_day=[overnight_qty[i]-overnight_qty[i+1] for i in range(idx,len(overnight_qty)-1)]
        overnight_penalty = self.overnight_weight[0] * sum(overnight_per_day[:3]) + self.overnight_weight[1] * sum(overnight_per_day[3:6])+self.overnight_weight[2] * sum(overnight_per_day[6:10])+self.overnight_weight[3] * sum(overnight_per_day[10:])
