...
import argparse
from agents.replenish_agent import ReplenishAgent
from pathlib import Path
from trainer import TrainerConfig
from task import task_dict
from utils.io import read_json, save_json
from utils.log import logging_once
from models import modelconfig_dict


def setup_cpu(rank, world_size, master_addr="localhost", master_port="12355"):
    """
    在CPU上初始化分布式环境
    """
...
def run_cpu_training(rank, world_size, config, master_addr="127.0.0.1", master_port="12355"):
    """
    在CPU上运行分布式训练
    """
    # 初始化分布式环境
    ...
    # 更新配置以支持分布式训练
    # cfg["distributed"] = cfg.get("distributed", True)
    # 通过world_size的数量
    ...
    # 创建智能体并训练
    agent = ReplenishAgent(None, config)
    # 训练前先创建一次json文件，防止实验设置无法复现;
    agent.make_json_file(config.conf_path)  # TODO: save json when save model
    agent.train()

    # 训练完成后保存模型（只有主进程会实际保存）
    agent.save_model(config.policy_model_filename)
    # 训练完成后保存json文件
    agent.make_json_file(config.conf_path)
    # 转存onnx文件
    agent.save_to_onnx(config.onnx_path)
    # 清理分布式环境
    cleanup()
...
def arg_parser():
    ...
    parser.add_argument(
        ...
        help="原始训练数据,如果是文件名就在数据文件夹查找,是绝对路径就",
    )
    parser.add_argument(
        "--valid_data_path",
        type=str,
    ...
    parser.add_argument(
        "--use_discount_reward_norm", type=str2bool, default=argparse.SUPPRESS, help="是否使用reward归一化"
    )
    ...
    parser.add_argument("--checkpoint_name_policy", type=str, default='', help="policy的checkpoint路径")
    parser.add_argument("--checkpoint_name_value", type=str, default='', help="value的checkpoint路径")
    parser.add_argument(
        "--action_ls",
        type=lambda x: list(map(float, x.split(","))),
        default=argparse.SUPPRESS,
        help="标签的映射关系,一般都比较长,不建议在args中传入",
    )
...
def main():
    ...
    #     save_json(config.__dict__, config.conf_path, indent=4)
    ...
    if config.world_size == 1:
        run_cpu_training(0, 1, config, config.master_addr, config.master_port)  # 单进程训练
    else:
