...
class ReplenishAgent:
    def __init__(self, replenish_model, config):
        ...
        self.valid_env = ReplenishEnv(
            config.valid_data_path, # 这里要加进来
            # rts_day=config.rts_days,
            # coverage_weight=config.coverage_weight,
            # rts_weight=config.rts_weight,
            # overnight_weight=config.overnight_weight,
            ...
            # world_size=config.world_size,
            cfg=config,
        ) if hasattr(config, "valid_data_path") else None

        self.use_train_env() # 默认使用train_env作为当前env
        ###输入的数据
        ###商品清单
        # self.datasets = self.env.datasets
        ...
        if self.is_checkpoint:
            if os.path.exists(self.policy_model_filename):
                self.start_episode = self.load_checkpoint(self.policy_model_filename,self.value_model_filename)
        ...
        if self.is_master:
            self.logger = SummaryWriter(log_dir=config.tb_log_path)  # tensorboard的log加载器
        if not config.distributed or self.is_master:
            print("initialize done")
            self.total_rolling_time = max(self.train_env.datasets.get_end_date_map().values())
    ...
    def use_train_env(self):
        """把指针指向训练环境，并同步相关变量"""
        self.env = self.train_env
        # 商品清单
        self.avg_item_qty_7d_map = self.train_env.datasets.avg_item_qty_7d_map
        self.sku_id_ls = self.train_env.datasets.sku_ids()
        self.predict_leadtime_day = self.train_env.datasets.predict_leadtime_day
        self.leadtime_map = self.train_env.datasets.leadtime_map
        self.initial_stock_map = self.train_env.datasets.get_initial_stock_map()
        self.end_date_map = self.train_env.datasets.get_end_date_map()
        self.total_sales = self.train_env.datasets.total_sales  # 这里是所有进程的总销量
        self.sales_map = self.train_env.datasets.sales_map
        self.static_state_map = self.train_env.datasets.get_static_state_map()
        self.dq_map = self.train_env.datasets.dq_map
        self.predicts_map = self.train_env.datasets.predicts_map


    def use_valid_env(self):
        """把指针指向验证环境，并同步相关变量"""
        if self.valid_env:
            self.env = self.valid_env
            self.avg_item_qty_7d_map = self.valid_env.datasets.avg_item_qty_7d_map # 先注释掉
            self.sku_id_ls = self.valid_env.datasets.sku_ids()
            self.predict_leadtime_day = self.valid_env.datasets.predict_leadtime_day
            self.leadtime_map = self.valid_env.datasets.leadtime_map
            self.initial_stock_map = self.valid_env.datasets.get_initial_stock_map()
            self.end_date_map = self.valid_env.datasets.get_end_date_map()
            self.valid_total_sales = self.valid_env.datasets.total_sales
            self.sales_map = self.valid_env.datasets.sales_map
            self.static_state_map = self.valid_env.datasets.get_static_state_map()
            self.dq_map = self.valid_env.datasets.dq_map
        return
    ...
    def gen_network_state(self, state_dict, sku, day_idx):
        ...
        if self.state_label == "with_rts_split":
            ...
            # l = len(self.static_state_map[sku])
            # print(f'len:{l}')
            new_state = [
                    start_stock,
                    transit_stock[0],
    ...
    def update(self):
        ...
        # 在分布式训练中使用DistributedSampler，一条轨迹的数据没有全用，而是进行了采样
        # if self.distributed:
        ...
        for episode in range(self.start_episode, self.max_episodes):
            ...
            while not done_all:
                ...
                for sku in self.sku_id_ls:
                    ...
                    if self.config.model_name == "ppo_continue_action":
                        ...
                        sku_action_map["abo_action"] = float(action)  # TODO: abo_action result
                    else:
                ...
                """环境step"""
                st = time.time()
                rolling_state_map, result_map, train_bind, train_rts = self.env.batch_step(action_map)  # 环境step，获得新的state相关信息和reward信息
                train_total_bind += train_bind
                train_total_rts += train_rts
                ...
                for sku in self.sku_id_ls:
                    if not done_map[sku]:
                        ...
                        safe_stock_gap += rolling_state_map[sku]["safe_stock_gap"]  # 记录安全库存差值

                if set(done_map.values()) == {True}:  ###所有商品都是done才能结束
                    done_all = True
                if not self.distributed or self.is_master:
                    self.episode_bar.update(1)
                    self.episode_bar.refresh()

            ####验证集进行模型评估
            ###商品network-state初始化，日期初始化
            if self.valid_env:
                self.use_valid_env() # 切换环境和相应的商品清单
                state_map = {sku: self.reset_network_state(sku) for sku in self.sku_id_ls}  ##idx:state
                if self.use_state_norm == 1:
                    for sku in self.sku_id_ls:
                        state_map[sku] = self.state_norm(state_map[sku]) # 调用Normalization实例的默认函数来归一化
                # 设置样本长度，方便一会的mean与std同步，验证集应该用不到？
                # self.state_norm.running_ms.set_sample_num(len(self.sku_id_ls))
                ...
                while not done_all:
                    ...
                    """环境step"""
                    st = time.time()
                    rolling_state_map, result_map, valid_bind, valid_rts = self.env.batch_step(action_map)  # 环境step，获得新的state相关信息和reward信息
                    valid_total_bind += valid_bind
                    valid_total_rts += valid_rts
                    ...
                    for sku in self.sku_id_ls:
                        if not done_map[sku]:
                            ...
                            # self.rewards_map[sku].append(reward)
                            # self.dones_map[sku].append(done)
                            valid_rts_qty += rolling_state_map[sku]["estimate_rts_qty"]  # 当前补货导致的总rts
                            valid_overnight_qty = rolling_state_map[sku]["estimate_overnight"]
                            valid_over_night += sum(valid_overnight_qty)
                            valid_total_rep += rolling_state_map[sku]["abo_qty"]
            ...
            if self.distributed:
                stats_tensor = torch.tensor(
                    [total_reward, float(over_night), float(total_rep),float(train_total_bind),float(train_total_rts),float(rts_qty),float(safe_stock_gap)], device=self.device
                )
                # 在所有进程上收集总和
                dist.all_reduce(stats_tensor, op=dist.ReduceOp.SUM)
                # 更新为全局统计值
                total_reward = stats_tensor[0].item()
                over_night = int(stats_tensor[1].item())
                total_rep = int(stats_tensor[2].item())
                train_total_bind = int(stats_tensor[3].item())
                train_total_rts = int(stats_tensor[4].item())
                rts_qty = int(stats_tensor[5].item())
                safe_stock_gap = int(stats_tensor[6].item())

                if self.valid_env:
                    valid_stats_tensor = torch.tensor(
                        [valid_total_reward, float(valid_over_night),float(valid_total_rep),float(valid_total_rts),float(valid_total_bind)], device=self.device
                    )
                    # 在所有进程上收集总和
                    dist.all_reduce(valid_stats_tensor, op=dist.ReduceOp.SUM)
                    # 更新为全局统计值
                    valid_total_reward = valid_stats_tensor[0].item()
                    valid_over_night = int(valid_stats_tensor[1].item())
                    valid_total_rep = int(valid_stats_tensor[2].item())
                    valid_total_rts = int(valid_stats_tensor[3].item()) # 每天的rts之和
                    valid_total_bind = int(valid_stats_tensor[4].item())

            # 只在主进程记录奖励历史
            if not self.distributed or self.is_master:
                self.logger.add_scalar("train_reward",total_reward,episode)
                self.logger.add_scalar("train_acc",train_total_bind/self.total_sales,episode)
                self.logger.add_scalar("train_rts",train_total_rts/train_total_bind,episode)
                self.logger.add_scalar("train_rep",total_rep,episode)
                self.logger.add_scalar("train_over_night",over_night,episode)
                self.logger.add_scalar("estimate_rts",rts_qty/train_total_bind,episode)
                # 训练过程中各类reward的变化，如果要修改reward，这里可能要对应更改
                self.logger.add_scalar("bind_reward",train_total_bind*self.config.lt_bind_weight,episode)
                self.logger.add_scalar("rts_penalty",rts_qty*self.config.rts_weight,episode)
                # self.logger.add_scalar("overnight_penalty",over_night*self.config.overnight_weight,episode)
                self.logger.add_scalar("safe_stock_penalty",safe_stock_gap*self.config.safe_stock_weight,episode)
                if self.valid_env:
                    self.logger.add_scalar("valid_reward",valid_total_reward,episode)
                    self.logger.add_scalar("valid_acc",valid_total_bind/self.valid_total_sales,episode)
                    self.logger.add_scalar("valid_rts",valid_total_rts/valid_total_bind,episode)
                    self.logger.add_scalar("valid_rep",valid_total_rep,episode)
                    self.logger.add_scalar("valid_over_night",valid_over_night,episode)
            ...
            if not self.distributed or self.is_master:
                ...
                if (episode + 1) % 50 == 0 and episode >= 199:
                    ...
                    value_model_epoch = os.path.join(self.config.outs_dir, f"repl_value_model_{episode + 1}.pth")
                    self.save_model(policy_model_epoch)
                    self.save_value_model(value_model_epoch)
    ...
    def load_checkpoint(self, policy_path,value_path):
        """加载模型和网络配置"""
        model_info = torch.load(policy_path, map_location=self.device)
        # 加载模型参数
        ...
        print(f"Checkpoint loaded from {policy_path}. Resuming from episode {start_episode}")

        value_model_info = torch.load(value_path, map_location=self.device)
        self.value.load_state_dict(value_model_info["state_dict"])
        # self.value_optimizer.load_state_dict(value_model_info["optimizer_state_dict"])
