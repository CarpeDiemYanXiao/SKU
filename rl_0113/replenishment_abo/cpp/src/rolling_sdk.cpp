...
ErrorCode validate_sku_input(const SkuInputInfo* input) {
    ...
    if (!input->estimate_rts_qty_list) return NULL_POINTER_ERROR;
...
ErrorCode roll_sku_one_day(SkuInputInfo* input, int sales, int* predicts, float multiplier, int abo_qty) {
    // 参数验证
    if (!input || !predicts) return NULL_POINTER_ERROR;
    ...
    try {
        ...
        }  //计算lt天期末库存，没考虑rts，期末库存偏高，补货量会偏低（线上rts不好获取，为了跟线上保持一致）

        // 计算补货量
        input->abo_qty = multiplier == 0 ? abo_qty : static_cast<int>(std::max(predicts[lead_time] * multiplier - end_of_stock, 0.0f));
        ...
        // 更新期末库存，这里已经扣减rts（也就是说，晚上扣减当天rts量）
...
ErrorCode step_one_sku(SkuInputInfo* input, bool evaluate, ReplenishmentCallback callback, int overnight_key) {
    ...
    result = roll_sku_one_day(input, input->sales[next_day_index], input->predicts[next_day_index], multiplier, input->abo_qty);
    if (result != SUCCESS) {
        return result;
    }
    ...
    try {
        ...
        for (size_t i = 1; i < total_days; i++) {
            ...
            // 模拟下一天
            result = roll_sku_one_day(&temp, input->sales[future_day_index], input->predicts[future_day_index], 0, 0);
            if (result != SUCCESS) {
                debug_print("Error in simulation for day", future_day_index, ":", getErrorMessage(result));
                break;
            }

            // 记录overnight、lead_time_bind、estimate_end_stock、estimate_rts_qty_list（for reward计算）
            input->overnight_list[i] = temp.ending_stock_list[overnight_key]; //overnight库存
            if (i == lead_time) {
                input->lead_time_bind = temp.bind_stock; //leadtime天bind量
                input->estimate_end_stock = temp.end_of_stock; // lt天期初库存
            }
            input->estimate_rts_qty_list[i] = temp.rts_qty;
        }
        input->estimate_rts_qty = temp.rts_qty; // rts天预估rts量
