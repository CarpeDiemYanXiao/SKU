...
typedef struct {
    ...
    int* estimate_rts_qty_list;      // 记录当前滚动情况下, 未来产生rts的列表
    ...
    int* sales;                      // 实际销量 [days]
    float multiplier;                // 补货乘数，直接传值替代 callback（避免 GIL 问题）
    ReplenishmentCallback callback;  // 保留 callback 字段以保持兼容性，但不再使用
} SkuInputInfo;
