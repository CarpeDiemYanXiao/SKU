...
def init_skus(rts_day, sku_ids, datasets: datasets.Datasets) -> ctypes.Array[SkuInputInfo]:
    ...
    for i, sku_id in enumerate(sku_ids):
        ...
        estimate_rts_qty_list_np = np.zeros(simulation_days, dtype=np.int32)
        ...
        sku_arrays = {
            ...
            "estimate_rts_qty_list": estimate_rts_qty_list_np,
        }
        memory_keeper.add_sku_arrays(str(sku_id), sku_arrays)
        memory_keeper.add([orders_np, order_returned_np, overnight_list_np, ending_stock_list_np, estimate_rts_qty_list_np])
        ...
        estimate_rts_qty_list = estimate_rts_qty_list_np.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        ...
        skus[i] = SkuInputInfo(
            ...
            estimate_rts_qty_list=estimate_rts_qty_list,
...
def reset_skus(skus: ctypes.Array[SkuInputInfo]):
    ...
    for i in range(len(skus)):
        ...
        sku.multiplier = 0.0  # 必须重置，否则abo_qty模式下C++会使用旧的multiplier值
        ...
        if sku_arrays:
            ...
            sku_arrays["estimate_rts_qty_list"].fill(0)
        else:
            ...
            for j in range(orders_size):
                ...
                sku.estimate_rts_qty_list[j] = 0

    return skus


def rolling(skus: ctypes.Array[SkuInputInfo], evaluate, action_type, action_map):
    sku_count = len(skus)

    for i in range(sku_count):
        if action_type == "abo_qty":
            skus[i].abo_qty = action_map[skus[i].id_str].get("abo_action") # 保持原类型
            skus[i].multiplier = 0.0  # 必须设为0，否则C++会使用multiplier公式而非abo_qty
            continue
        ...
        skus[i].multiplier = float(get_action(0))
        with open('/Users/weibin/Desktop/weibin_projects/replenishment_conti/log_info/multiplier_debug.log', 'a') as f:
            f.write(f"[rank 0] sku_id: {skus[i].id_str}, multiplier: {skus[i].multiplier}\n")
