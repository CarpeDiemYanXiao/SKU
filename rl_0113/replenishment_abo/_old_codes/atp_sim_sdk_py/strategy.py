...
class StrategyA(Strategy):
    def bind_stock(self, sku: SKU, sales: int) -> int:
        return min(sku.begin_stock + sku.order_pool.get_rep(sku.day_index), sales)
    ...
    def calculate_rep(self, sku: SKU, predicts: list[float], multiplier: float) -> int:
        ...
        for i in range(1, sku.lead_time):
            ...
            r = math.ceil(max(predicts[ld] * multiplier - max(f + r - sum(predicts[0 : ld - 1]), 0), 0))
        return r


class StrategyB(Strategy):
    def bind_stock(self, sku: SKU, sales: int) -> int:
        return int(min(sku.begin_stock + sku.today_arrived, sales))

    def end_stock(self, sku: SKU) -> int:
        return int(max(sku.begin_stock + sku.today_arrived - sku.bind_stock, 0))
    ...
    def calculate_rep(self, sku: SKU, predicts: list[float], multiplier: float) -> int:
        ...
        for i in range(1, sku.lead_time):
            f = p  # Fn
            p = max(f + sku.arriving_stock(sku.day_index + i) - predicts[i], 0)  # Pn
        return math.ceil(max(predicts[sku.lead_time] * multiplier - p, 0))
...
class StrategyC(Strategy):
    ...
    def calculate_rep(self, sku: SKU, predicts: list[float], multiplier: float) -> int:
        ...
        return math.ceil(max(predicts[sku.lead_time + 1] * multiplier - p, 0))
