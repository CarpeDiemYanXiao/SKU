...
from atp_sim_sdk_py import replenish_env, replenish_scene, rolling_env, strategy


class ReplenishIndependentAgent:
    ...
    def update(self):
        ...
        for _ in range(self.k_epochs):
            ...
            m = Categorical(probs)
            new_logprobs = m.log_prob(actions)  ## # 计算采样动作的对数概率
    ...
    def train(self, max_episodes=3000):
        ...
        for episode in range(max_episodes):
            ...
            state = self.env.reset()  ###需要对所有sku进行初始化
            s = {sku.idx: sku.reset() for sku in self.env.skus}
            done = False
            total_reward = 0
            while not done:
                for sku in self.skus:
                    action = self.select_action(state)
                    step_result = self.env.step(action, sku)
...
if __name__ == "__main__":
    ...
    agent = ReplenishIndependentAgent(env, device="cpu")
    agent.train(5)
